...
SYSTEM_PROMPT = """Write the next line of an audio description track for this video.
Describe the most important new visual information not already clear from the dialogue or previous descriptions, using established names where helpful.
Ground the description in the frames and context; do not invent unseen events, motives, or intent.
Return only concise narration that fits the gap, or SKIP if no description is needed."""
...
def find_gaps(
    ...
    speech_spans = (
        ...
        for span in (
            segment.speech_spans
            or ((segment.start, segment.end),)
        )
    )
    candidates = (
        _invert(speech_spans, duration)
...
def _transcript_context(
    ...
    if before:
        if since is None:
            ...
            label = "Since the start of the video"
        else:
            label = "Since the previous description"
        lines.append(f"{label}: " + " ".join(item.text for item in before[-12:]))
    if around:
        lines.append(
            "Around the gap: "
            + " ".join(item.text for item in around)
        )
    if after:
        lines.append(
            "After the gap: "
    ...
    return "\n".join(lines) if lines else "No nearby dialogue."
...
def build_description_prompt(
    ...
    *,
    ...
    if catchup_count and not previous:
        frame_note = (
            f"First {catchup_count} frame(s): earlier context from the video opening. "
            f"Remaining frames: this gap."
        )
    elif catchup_count:
        frame_note = (
            f"First {catchup_count} frame(s): earlier context since the previous "
            f"description. Remaining frames: this gap."
        )
    else:
        frame_note = "This gap; the first frame may be immediately before it."
    continuity = ""
    if same_shot_text:
        continuity = (
            '\nContinuity: no cut since the previous description, "'
            f'{same_shot_text}".'
        )
    return (
        f"Available gap: {gap.duration:.1f}s starting at "
        f"{vtt.format_timestamp(gap.start)}; maximum "
        f"{word_budget(gap.duration)} words.\n\n"
        f"Dialogue:\n{_transcript_context(segments, gap, since=since)}\n\n"
        f"Previous audio descriptions:\n{prior}\n\n"
        f"Frames: {frame_note}{continuity}"
    )
...
def _content_filter_fallback_frames(
    frames: Sequence[Path], catchup_count: int
) -> list[Path]:
    gap_frames = list(frames[catchup_count:])
    if catchup_count and gap_frames and len(gap_frames) < len(frames):
        return gap_frames
    if len(frames) == 1 and not catchup_count:
        return list(frames)
    if len(frames) > 1:
...
def _content_filter_context(
    segments: Sequence[Segment], gap: Gap
) -> list[Segment]:
    before = [
        segment
        for segment in segments
        if segment.end <= gap.start and segment.text
    ]
    around = [
        segment
        for segment in segments
    ...
    after = [
        segment
        for segment in segments
        if segment.start >= gap.end and segment.text
    ]
    return [
        *before[-1:],
        *around,
        ...
        segments = [
            Segment(
                ...
                tuple(
                    ...
                    for start, end in getattr(item, "speech_spans", ())
                ),
            )
            for item in items
        ]
...
def _generate_ad(
    ...
    for index, gap in enumerate(gaps, start=1):
        ...
        try:
            ...
            fallback_frames = _content_filter_fallback_frames(
                frames, catchup_count
            )
            if (
                not openai_transport.is_content_filter_error(exc)
                or not fallback_frames
            ):
                warnings.append(
                    ADWarning("model", gap.start, gap.end, str(exc))
                )
                _progress(progress, "describing", index, len(gaps))
                continue
            fallback_prompt = build_description_prompt(
                gap,
                _content_filter_context(segments, gap),
                spoken,
                catchup_count=0,
                same_shot_text=same_shot_text,
            )
            try:
                ...
                warnings.append(
                    ADWarning(
                        ...
                        gap.start,
                        gap.end,
                        "The full frame set was rejected by the content "
                        f"filter; generated from {len(fallback_frames)} "
                        "gap frame(s).",
                    )
                )
