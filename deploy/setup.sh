...
echo "   • Microsoft Foundry resource for Content Understanding, GPT, and Speech"
...
FOUNDRY_NAME=$(echo "$DEPLOYMENT_OUTPUT" | jq -r '.foundryName.value')
FOUNDRY_SPEECH_ENDPOINT=$(echo "$DEPLOYMENT_OUTPUT" | jq -r '.foundrySpeechEndpoint.value')
...
FOUNDRY_KEY=$(az cognitiveservices account keys list --name "$FOUNDRY_NAME" --resource-group "$RESOURCE_GROUP_NAME" --query "key1" --output tsv)
...
VITE_FOUNDRY_RESOURCE=${FOUNDRY_NAME}
VITE_FOUNDRY_KEY=${FOUNDRY_KEY}
VITE_FOUNDRY_SPEECH_ENDPOINT=${FOUNDRY_SPEECH_ENDPOINT}
